    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col">
                        ...
                        <div class="card shadow">
                            <div class="card-body">
                                <br>
                                <h5 class="card-title">Daftar Barang yang akan dilelang</h5>
                                <br>
                                <form action="{{route('lelang.filter') }}" method="GET">
                                    <div class="row">
                                ...
                                <div class="container mt-4">
                                    <form action="{{ route('lelang.sendToPimpinan') }}" method="POST">
                                        @csrf
                                        <table class="table table-bordered">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td style="text-align: center;">{{ $loop->iteration }}. <input type="checkbox" name="selected[]" value="{{ $item->idbarang }}"></td>
                                                    <td>{{ $item->kode_barang }}</td>
                                                    <td>{{ $item->nama }}</td>
                                                    <td>{{ $item->kategori }}</td>
                                                    <td>{{ $item->type }}</td>
                                                    <td>{{ $item->ruang ? $item->ruang->nama_ruang: '-' }}</td>
                                                    <td>{{ $item->harga }}</td>
                                                    ...
                                                    <td class="gap-3">
                                                        @if($item->status == 0)
                                                        <a href="{{url('/admin/edit/'.$item->idbarang)}}" class="btn btn-outline-warning shadow-sm"><i class="bi bi-pencil-square"></i> Edit</a>
                                                        @endif
                                                    </td>
                                                </tr>
                                                @endforeach
                                        ...
                                        <hr>
                                        <button type="submit" class="btn btn-primary shadow">Kirim ke Pimpinan</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <!-- <script>
                    function bukaPopUp() {
                        window.open("/admin/lelang/email", "popUpWindow", "width=600, height=400, scrollbars=yes");
                    }
                </script> -->
            </div>
        </div>      
